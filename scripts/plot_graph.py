...
def plot_experiment(df, dataset, experiment):
    ...
    if len(filtered_subset) < len(subset):
        print(
            f"Filtered {len(subset) - len(filtered_subset)} outliers (Growth Spike) from {experiment}"
        )
    ...
    )  # Use a nice style if available, else default
    ...
    plt.plot(
        subset[x_col],
        subset["Ratio"],
        label="Approximation Ratio (FW/Greedy)",
        marker="^",
        color="purple",
    )
