...
def f2(t, Sigma, v, delta, x0, cg_maxiter=None, cg_tol=1e-5):
    ...
    t_sqr = t * t
    t_sqr[t_sqr < 1e-6] = (
        1e-6  # For numerical stability, small values are mapped to 1e-8.
    )
...
def f_grad(t, Sigma, v, delta, gamma, cg_maxiter=None, cg_tol=1e-5):
...
def fw(
    ...
    while not stop:
        ...
        grad, gamma = f_grad(
            t, Sigma, v, delta, gamma, cg_maxiter=cg_maxiter, cg_tol=cg_tol
        )
...
def grid_fw(
    ...
    if count < patience:
        print(
            "WARNING: Algorithm terminated after maximum epochs without convergence! Consider increasing the number of epochs."
        )
