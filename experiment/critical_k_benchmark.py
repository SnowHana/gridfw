...
def main():
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python experiment/critical_k_benchmark.py [DATASET_NAME] [OUTPUT_FILENAME] [TARGET_SPEEDUP]"
        )
        sys.exit(1)
    ...
    if p_full < 50:
        ...
        p_values = np.linspace(
            max(2, int(p_full * 0.01)),
            p_full,
            num=min(NUM_POINTS, p_full // 2),
            dtype=int,
        )
    else:
    ...
    for run_id in range(NUM_REPEATS):
        ...
        for p in p_values:
            ...
            print(
                f"  > p={p}: k={best_k}, Speedup={entry['Speedup_At_k']:.4f}, Ratio={entry['Ratio']:.4f}, Steps={entry['Steps']}, Samples={entry['Samples']}"
            )
